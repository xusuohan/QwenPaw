# PACK\_PROGRESS\_BASELINE — 打包优化总基准（所有新会话的统一起点）
...
- 仓库：QwenPaw-aixcore（Python 3.10、FastAPI + agentscope\_runtime 多 Agent 桌面应用）。
...
1. **wheel\_build.sh**：源码 hash（`src/**/*.py` + `pyproject.toml` + `console/src/**/*.ts` + `package-lock.json`）未变 → 跳过 npm ci + npm build + python -m build。
2. **build\_common.py**：wheel 内容 hash 未变 + conda env 存在 → 跳过 conda create + pip install 等全部阶段，仅运行 conda-pack。
3. **build\_portable.sh**：清理 dist/ 时保留 wheel 和 sdist，不破坏下次构建的缓存条件。
...
| `e8a5474b` | 修复：跨进程 `time.monotonic()` → `time.time()`，CLI 封装（begin\_stage/finish\_stage） |
...
| `288128ba` | CI workflow：macOS x86\_64 job + Windows smoke test + profiling uploads       |
...
- CI 矩阵扩展：macOS arm64 + x86\_64 + Linux + Windows（含 smoke test）
...
| 阶段                                           | 耗时    | 说明         |
| -------------------------------------------- | ----- | ---------- |
| wheel\_build                                 | 0s    | 跳过（源码未变）   |
| conda\_create                                | 0s    | 跳过（env 存在） |
| pip\_install                                 | 0s    | 跳过（env 存在） |
...
| 阶段                | 耗时                  | 占比        |
| ----------------- | ------------------- | --------- |
| wheel\_build      | \~120s              | 12%       |
| conda\_create     | 16.5s               | 1.6%      |
| **pip\_install**  | **897.6s (15 min)** | **89.4%** |
| verify\_certifi   | 3.0s                | 0.3%      |
| pip\_uninstall    | 6.9s                | 0.7%      |
| pip\_cache\_purge | 3.1s                | 0.3%      |
| conda\_fix        | 11.9s               | 1.2%      |
| conda\_pack       | 64.0s               | 6.4%      |
| **总计**            | **\~16.5 min**      | <br />    |
...
| chromadb\_rust\_bindings | 52 MB  | transitive（reme-ai）      |
| transformers             | 43 MB  | ml extra                 |
| pandas                   | 38 MB  | transitive（reme-ai）      |
| alibabacloud\_dingtalk   | 38 MB  | 核心依赖                     |
...
| lark\_oapi               | 19 MB  | channels extra           |
...
1. 在 Windows 环境运行 `make portable-windows 或 （make portable）`（或 `powershell -File scripts/pack/build_win_portable.ps1`）
...
**CI**：`.github/workflows/build-portable.yml`（macOS arm64/x86\_64 + Linux + Windows）
...
1. **wheel\_build.sh 的 npm build 是非确定性的**：即使源码不变，`npm ci && npm run build` 产出的 wheel 内容也不同（时间戳等）。已通过源码 hash 跳过重建解决。**不要删除** **`.cache/wheel_source_hash`**。
