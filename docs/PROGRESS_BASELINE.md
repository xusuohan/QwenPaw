...
- 本次性能优化提交范围：`9dc5c3b0`（spec）→ `fc91941d`（HEAD），共 ~41 个提交。
...
**spec 各章节归属**（防止重复/遗漏）：
- §2.1/2.2 基石 → Phase 1 ✅
- §3.1 遥测后台 + §3.3 语言硬编码 → Phase 3 ✅；§3.2 迁移戳幂等 → Phase 4e ✅；§3.3 load_envs 延迟到 lifespan → Phase 4j ✅；§3.4 restore stat 精简 → Phase 4f ✅；§3.5 Workspace.start 冗余去重 → Phase 4h ✅；§3.6 → 见 §6 待办
...
### Phase 4h — §3.5 Workspace.start 冗余去重（`fc91941d`）
- `agents/skills_manager.py`：`ensure_skill_pool_initialized` 加模块级 `_skill_pool_initialized` 标记；首次调用后所有后续调用立即返回 `False`（零 IO）。Skill pool 是全局共享目录，N agent 启动 → 1 次初始化 + N-1 次 no-op。
- `app/workspace/workspace.py`：`_migrate_legacy_weixin_data` 加 ClassVar `_weixin_migrated_workspaces`（`set[str]`）；以 workspace 路径为 key 去重。同 workspace 重启时跳过 3 迁移函数（chats/jobs/sessions），不同 workspace 独立迁移。
...
- **依赖链分析**：`load_envs_into_environ()` 在两处调用——`__init__.py:43`（`import qwenpaw` 时，`constant.py` 导入**之前**）+ `_app.py:67`（`constant.py` 导入**之后**，`overwrite=False` 使此调用恒为空操作）。`__init__.py:43` 是正确性关键（26+ 模块级 env-backed 常量依赖此顺序）；`_app.py:67` 是冗余 IO（文件锁获取 + 目录扫描 + 文件读取 + 解密）。
- **改动**：移除 `_app.py:67` 冗余模块级调用 → lifespan Phase 0 加 `_load_envs_defer_enabled()` 门控的 safety-net `load_envs_into_environ()` 调用（guard 边缘场景：uvicorn reload 子进程可能未走 `__init__.py`）→ `__init__.py:43` 保留不动。
- **kill-switch**：`QWENPAW_PERF_LOAD_ENVS_DEFER`（默认开，`0/false/no/off` 关）；关闭后 lifespan 跳过 safety-net 调用（行为与移除前一致——`__init__.py:43` 已加载）。
- **风险说明**：深度方案（移除 `__init__.py:43` + 26+ 常量改 lazy 求值）为高风险大范围重构，defer 至需要时再做。
- 测试：`tests/unit/app/test_load_envs_defer.py`（14：flag 默认/禁用/启用值/AST 验证模块级调用已移除/`__init__.py:43` 保留验证/lifespan 源码含 safety 调用 + flag 门控）。全 209 utils+app 回归测试通过。
...
**建议下一会话**：§3.6.2 import 预读真机测量（需用户在 USB/exFAT 真机验证）。所有 spec 代码任务已完成。
...
**结构性改动**：`app/_app.py`（lifespan：runner 接入 + 遥测后台 + 孤儿清理 + run_migrations 封装 + §5.3 stop_queue_listeners + §3.3 load_envs safety-net）、`app/channels/manager.py`（replace_channel 死锁修复）、`cli/main.py`（prefetch 接入）、`agents/model_factory.py`（§4.1 模型客户端指纹缓存 + 接入工厂两分支）、`app/runner/repo/json_repo.py`（§4.3 chats 读缓存 + 写穿透异步写）、`agents/context/light_context_manager.py`（§4.5 Part A tool result 溢写 async + atomic）、`app/migration.py`（§3.2 戳门控 + run_migrations 封装）、`backup/_utils/safe_swap.py` + `_mount_swap.py`（§3.4 scandir 优化）、`security/secret_store.py`（§5.4 原子写）、`agents/skills_manager.py` + `app/workspace/workspace.py`（§3.5 进程级 once 标记）、`utils/logging.py`（§5.3 QueueHandler+QueueListener 封装 + 双文件常量）、`cli/desktop_cmd.py`（§5.3 desktop 文件日志 + atexit）、`cli/doctor_checks.py`（§5.3 指向 LOG_BACKEND_PATH）
**构建脚本**：`scripts/pack/build_{linux,macos,win,win_portable}.{sh,ps1}`（均 `--invalidation-mode checked-hash`）
**测试**：`tests/unit/utils/{test_atomic_io,test_background_tasks,test_import_prefetch,test_telemetry_marker,test_log_queue}.py`、`tests/unit/app/{test_runner_session,test_auth_cache,test_json_repo,test_migration_stamp,test_load_envs_defer}.py`、`tests/unit/channels/test_channel_manager.py`、`tests/unit/agents/{test_model_client_cache,test_skill_pool_once}.py`、`tests/unit/app/test_chats_cache.py`、`tests/unit/agents/context/test_tool_result_offload.py`、`tests/unit/backup/test_restore_scandir.py`、`tests/unit/workspace/test_weixin_migration_once.py`
